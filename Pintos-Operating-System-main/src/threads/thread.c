...
#ifdef USERPROG
/* Allocate the new file descriptor for a new_file. and return valid desc or -1*/
int
allocate_fd (struct file *new_file)
{
  struct thread *curr_thread = thread_current ();
  if (curr_thread->next_fd >= FDCOUNT_LIMIT){
    return -1;}
  int assigned_fd = curr_thread->next_fd++;
  curr_thread->fd_table[assigned_fd] = new_file;
  return assigned_fd;
}

/* Close and remove the file that is associated with file_descriptorr. */
void
close_fd (int file_desc)
{
  struct thread *curr_thread = thread_current ();
  if (file_desc >= 2 && file_desc < FDCOUNT_LIMIT && curr_thread->fd_table[file_desc] != NULL)
    {
      file_close (curr_thread->fd_table[file_desc]);
      curr_thread->fd_table[file_desc] = NULL;
    }
}


/* Fetch the file that uis associated with file_descriptor, else null if invalid */
struct file *
fetch_file_by_fd (int file_desc)
{
  struct thread *curr_thread = thread_current ();
  if (file_desc < 0 || file_desc >= FDCOUNT_LIMIT)
    return NULL;
  return curr_thread->fd_table[file_desc];
}
...
/*Thread modifications to handle in userprograms*/
#ifdef USERPROG
