...
#ifdef USERPROG
  #define FDCOUNT_LIMIT 128    /* Max open files in process */

  /* File‐descriptor functions that are used in thread.c */
  int allocate_fd (struct file *new_file);
  struct file *fetch_file_by_fd (int file_desc);
...
struct thread
  {
    ...
    struct list child_threads;       /* List of this thread’s children */
    struct list_elem child_elem;     /* Element that is in in parent’s children list */

    struct semaphore semaphore_load;   /* Parent waits till child is loaded */
    bool load_success;               /* When loaded successfully*/
    struct file *executable_file;   /* The executable file*/

      /* Sync for wait/exit */
    struct semaphore semaphore_exit;      /* Parent waits until child exits */
    bool waited_on; 
